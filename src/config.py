...
OUTPUT_TRANSCRIPT_DIR = "./output/raw/"      # 生成されたwhisperの生データファイル(json形式)
OUTPUT_TEXT_DIR       = "./output/text/"            # 生成された全てのテキストをまとめたファイル(txt形式)
OUTPUT_AUDIO_DIR      = "./output/audio/"           # 動画から抽出した一時的な音声ファイル(m4a形式)

# --- Whisper ---
DEFAULT_MODEL_SIZE = "base"  # モデルサイズ（速度優先: tiny/base、精度優先: small/medium/large）

# --- 校正AI（DeBERTa 設定値の追加） ---
DEBERTA_MODEL_NAME = "ku-nlp/deberta-v2-base-japanese"  # 文脈校正に使用する日本語DeBERTaモデル名
...
# --- クリーンアップ設定 ---
REMOVE_TEMP_AUDIO = False  # 動画から抽出した一時的な m4a 音声ファイルを、処理終了後に自動削除するかどうか
